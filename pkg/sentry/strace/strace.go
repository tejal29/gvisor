...
func (i *SyscallInfo) pre(t *kernel.Task, args arch.SyscallArguments, maximumBlobSize uint) []string {
	...
	for arg := range args {
		...
		case SetOptVal:
			output = append(output, sockOptVal(t, uint32(args[arg-2].Uint64()), uint32(args[arg-1].Uint64()), args[arg].Pointer(), uint32(args[arg+1].Uint64()), maximumBlobSize))
		case SockAddr:
...
func (i *SyscallInfo) post(t *kernel.Task, args arch.SyscallArguments, rval uintptr, output []string, maximumBlobSize uint) {
	for arg := range output {
		...
		case GetOptVal:
			output[arg] = getSockOptVal(t, uint32(args[arg-2].Uint64()), uint32(args[arg-1].Uint64()), args[arg].Pointer(), args[arg+1].Pointer(), maximumBlobSize, rval)
		case SetOptVal:
