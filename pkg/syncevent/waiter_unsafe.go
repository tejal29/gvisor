...
// pending events. It does not affect the set of pending events; instead,
// callers must call w.Ack() to do so.
//
// Precondition: Only one goroutine may call Wait or WaitFor at a time.
...
// set of pending events; instead, callers must call w.Ack() to do so.
//
// Precondition: Only one goroutine may call Wait or WaitFor at a time.
